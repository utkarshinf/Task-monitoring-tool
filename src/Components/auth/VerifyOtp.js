     ...
     <section className="login_content d-flex justify-content-lg-end align-items-center position-relative">
            ...
            <img src={logo2} alt="Reality Assistant" className="position-absolute top-0 start-50 translate-middle-x mt-5"/>
                <form action="" className="w-100 px-5">
                    ...
                    <p className="text-black-50 mb-5 text-center small_text">Enter the 4 digits code that you received on your email.</p>
                        <div className="mb-4">
                            <p className="text-center">Verification code</p>
                            <div className="d-flex justify-content-center">
                            <input type="number" className="border border-black-50 border-2 border-bottom-0 p-2 rounded-2 otp-box me-3" />
                            <input type="number" className="border border-black-50 border-2 border-bottom-0 p-2 rounded-2 otp-box me-3" />
                            <input type="number" className="border border-black-50 border-2 border-bottom-0 p-2 rounded-2 otp-box me-3" />
                            <input type="number" className="border border-black-50 border-2 border-bottom-0 p-2 rounded-2 otp-box" />
                            </div>
                        </div>
                        <div className="d-flex justify-content-center">
                            <button className="text-uppercase  px-5 py-2 border-0 primary_btn rounded-2">VERIFY</button>
                        </div>
                </form>
            </div>
        </section>
